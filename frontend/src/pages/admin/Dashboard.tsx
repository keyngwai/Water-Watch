...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (statsQuery.isError) {
      getApiError(statsQuery.error).then(msg => toast.error(`Failed to load stats: ${msg}`));
    }
  ...
  useEffect(() => {
    if (recentReportsQuery.isError) {
      getApiError(recentReportsQuery.error).then(msg => toast.error(`Failed to load reports: ${msg}`));
    }
    ...
    <Layout>
     {/* <button
      ...
      <div style={styles.kpiGrid}>
        {[
          { label: 'Total Reports', value: totalReports, icon: '', color: 'var(--accent-color)', bg: 'var(--card-bg)' },
          { label: 'Awaiting Review', value: statusMap['reported'] || 0, icon: '', color: '#d97706', bg: 'var(--card-bg)' },
          { label: 'Work In Progress', value: statusMap['in_progress'] || 0, icon: '', color: '#7c3aed', bg: 'var(--card-bg)' },
          { label: 'Successfully Fixed', value: statusMap['resolved'] || 0, icon: '', color: '#10b981', bg: 'var(--card-bg)' },
        ].map((kpi) => (
          <div key={kpi.label} style={{ ...styles.kpiCard, background: kpi.bg, borderLeft: `4px solid ${kpi.color}` }}>
          ...
          {stats?.byStatus.map((item) => {
            ...
            return (
              <div key={item.status} style={styles.barRow}>
                <span style={{ width: '110px', fontSize: '13px', color: 'var(--text-color)', fontWeight: 500 }}>
                  {STATUS_LABELS[item.status as keyof typeof STATUS_LABELS]}
                </span>
                <div style={styles.barTrack}>
                  <div style={{ ...styles.barFill, width: `${pct}%`, background: color }} />
                </div>
                <span style={{ width: '40px', textAlign: 'right', fontSize: '13px', color: 'var(--muted-text)' }}>{count}</span>
              </div>
            );
          })}
        </div>
        ...
        <div style={styles.panel}>
          ...
          {stats?.byCategory.slice(0, 6).map((item, i) => (
            <div key={item.category} style={styles.categoryRow}>
              ...
              <span style={{ flex: 1, fontSize: '13px', color: 'var(--text-color)', fontWeight: 500 }}>
      ...
      <div style={styles.twoCol}>
            ...
            return (
              <div key={item.county} style={styles.countyRow}>
                ...
                <span style={{
                  flex: 1,
                  fontSize: '13px',
                  color: isUserCounty ? 'var(--accent-color)' : 'var(--text-color)',
        ...
        <div style={styles.panel}>
          ...
          <div style={{ padding: '20px', textAlign: 'center', color: 'var(--muted-text)' }}>
      ...
      <div style={styles.panel}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={styles.table}>
            <thead>
              <tr>
                {['Reference', 'Title', 'Category', 'Location', 'Status', 'Submitted', ''].map((h) => (
            ...
            <tbody>
              {recentData?.reports.map((report) => (
                <tr key={report.id} style={styles.tr}>
                  <td style={{ ...styles.td, fontFamily: 'monospace', fontSize: '12px', color: 'var(--muted-text)' }}>
                  ...
                  <td style={{ ...styles.td, fontSize: '13px', color: 'var(--muted-text)' }}>{report.county}</td>
                  <td style={styles.td}>
                    <span style={{
                      padding: '2px 8px',
                      borderRadius: '20px',
        ...
        <div style={styles.modalOverlay} onClick={() => setShowUpdateStatus(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ color: 'var(--text-color)', fontSize: '14px', marginTop: '4px' }}>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  barRow: { display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px', color: 'var(--text-color)' },
  barTrack: { flex: 1, height: '8px', background: 'var(--bg-color)', borderRadius: '4px', overflow: 'hidden', border: '1px solid var(--border-color)' },
  ...
  categoryRank: { fontSize: '12px', color: 'var(--muted-text)', width: '24px', flexShrink: 0 },
  categoryCount: {
    fontSize: '13px',
    fontWeight: 700,
    color: 'var(--accent-color)',
  ...
  countyRank: { fontSize: '12px', color: 'var(--muted-text)', width: '24px', flexShrink: 0 },
  countyCount: {
    fontSize: '13px',
    fontWeight: 700,
    color: '#10b981',
  ...
  td: { padding: '12px 12px', color: 'var(--text-color)' },
  categoryTag: {
    fontSize: '12px',
    background: 'var(--bg-color)',
    color: 'var(--muted-text)',
